...
if(isset($_POST['reg123']) && $_POST['reg123']== 'Sign-up'){
    ...
    if(isset($correctName) && isset($correctEmail) && isset($correctAge) && isset($crrGender) && isset($crrHobbies) && isset($crrcities)){
        echo "<script> alert('Successfully done') </script>";
        $name = $email = $age = $gender = $hobbies = $country = null;
        $correctName = $correctEmail = $correctAge = $crrGender = $crrHobbies =$crrcities = null;
    }
}
?>
                 ...
                 <!-- Gender Radio Area -->
                  ...
                  <!-- CheckBox Area -->
                   <div class="mb-1 <?= isset($errHobbies) ? 'border border-danger' : null?> rounded position-relative" >
                    ...
                    <div class="form-check form-check-inline">
                        <label for="Writing">
                            <input type="checkbox" class="form-check-input" name="hobbies[]" value="Writing" <?= isset($crrHobbies) && in_array("Writing", $crrHobbies) ? "checked" : null?>>Writing
                        </label>
                    </div>
                    <!-- Coading -->
                    <div class="form-check form-check-inline">
                        <label for="Writing">
                            <input type="checkbox" class="form-check-input" name="hobbies[]" value="Coading" <?= isset($crrHobbies) && in_array("Coading", $crrHobbies) ? "checked" : null?>>Coading
                        </label>
                    </div>
                    <!-- icon -->
                   ...
                   <!-- Country Area -->
                    <div class="mb-3 ">
                        <select name="country" class="form-select  <?= isset($errcities) ? 'is-invalid' : null?>" id="">
                            <option value="<?= $crrcities ?? null?>">Select Country</option>
                            <?php foreach($cities as $ct){?>
                                <option value="<?= $ct ?>" <?= isset($crrcities) && $crrcities == $ct ? "selected" : null?>><?= $ct?></option>
                            <?php } ?>
                        </select>
                    </div>
